...
func (bt *BTree) Put(key []byte, pos *data.LogRecordPos) (bool, *data.LogRecordPos) {
	if len(key) == 0 {
		return false, nil
	}
	...
	bt.lock.Unlock()
	if oldItem != nil {
		return true, oldItem.(*Item).pos
	}
	return true, nil
}

func (bt *BTree) Delete(key []byte) (bool, *data.LogRecordPos) {
	it := &Item{key: key}
	bt.lock.Lock()
